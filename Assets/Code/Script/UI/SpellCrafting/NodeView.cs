...
public class NodeView : MonoBehaviour, IPointerDownHandler, IBeginDragHandler, IDragHandler, IPointerClickHandler
{
    ...
    public void Init(SpellNodeSO data, GraphCanvasController canvas)
    {
        ...
        if (tmpLabels.Length > 1) { tmpLabels[1].text = $"[{data.complexityCost}]"; }
        ...
        var inT  = transform.Find("InputPort");
        var outT = transform.Find("OutputPort");
        if (outT == null) outT = transform.Find("OutPort");
        InputPort  = inT  != null ? inT .GetComponent<PortView>() : null;
        OutputPort = outT != null ? outT.GetComponent<PortView>() : null;
